...
static inline struct plugin_component_instance *plugin_comp_find (
	...
	unsigned int *iface_number)
{
	...
	while (qb_hdb_iterator_next (&plugin_component_instance_database,
		...
		qb_hdb_handle_put (&plugin_component_instance_database, component_handle);
	}

	return (NULL);
}

static inline int plugin_lib_loaded (
	char *library_name)
{
	struct plugin_component_instance *instance;
	void *instance_p = NULL;
...
static int interface_find_and_load (
	...
	const char *iface_name,
	int version,
	struct plugin_component_instance **instance_ret,
	unsigned int *iface_number)
{
	struct plugin_component_instance *instance;
	void *dl_handle;
	...
	scandir_entries = scandir (path,  &scandir_list, plugin_select_so, alphasort);
	if (scandir_entries > 0)
	/*
 ...
 */
#ifdef QB_SOLARIS
		comp_reg = dlsym (dl_handle, "corosync_plugin_component_register");
		comp_reg ();
#endif
		...
		*/
		if (g_component_handle != 0xFFFFFFFF) {
			qb_hdb_handle_destroy (&plugin_component_instance_database,
				g_component_handle);
